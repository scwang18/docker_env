...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: busybox
          image: 'registry.ipincloud.com/docker.io/busybox:1.28.0-glibc'
          command:
            - sleep
            - '36000'
          volumeMounts:
...
oc patch -n openshift-cluster-samples-operator  configs.samples.operator.openshift.io cluster -p '{"spec":{"samplesRegistry":"registry.ipincloud.com/registry.redhat.io"}}'  --type=merge
