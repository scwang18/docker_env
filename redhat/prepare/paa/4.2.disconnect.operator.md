...
打包好的安装包，在这里下载，目前最新版本是ocp 4.2.13： https://drive.google.com/drive/u/2/folders/1KYTMQnurBYwIz9Qh64_G2gILNiDP6C9e

其中包括如下类型的文件：
- ocp4.$version.tgz  这个文件包含了iso等安装介质，以及各种安装脚本，全部下载的镜像列表等。需要复制到宿主机，以及工具机上去。
- registry.$version.tgz  这个文件也是docker image registry的仓库打包文件，按照CCN教材编制（https://github.com/wangzheng422/docker_env/blob/master/redhat/ocp4/4.2.ccn.devops.deploy.md）, 以后计划再加入kafka，etcd，maridb，servicemesh，集群日志operator所用的镜像，所以比较小。但是要运行其他operator的话，需要先补充镜像。按照这里操作： https://github.com/wangzheng422/docker_env/blob/master/redhat/ocp4/4.2.add.image.md
- ocp4.tgz  这个文件这个文件包含了iso等安装介质，以及各种安装脚本。包含了多个版本的ocp4，目前打包的内容，包括了4.2.13, 4.2.12, 4.2.10.
- registry.tgz  这个文件也是docker image registry的仓库打包文件，按照CCN教材编制。包含了多个版本的ocp4，目前打包的内容，包括了4.2.13, 4.2.12, 4.2.10.
...
```
Success
Update image:  registry.ipincloud.com/ocp4/openshift4:4.2.8
Mirror prefix: registry.ipincloud.com/ocp4/openshift4

To use the new mirrored repository to install, add the following section to the install-config.yaml:

imageContentSources:
- mirrors:
  - registry.ipincloud.com/ocp4/openshift4
...
# firewall-cmd --zone=public --add-rich-rule='rule family="ipv4" forward-port to-addr="192.168.7.11" to-port="22" protocol="tcp" port="2022"'
# firewall-cmd --zone=public --add-masquerade

# firewall-cmd --runtime-to-permanent

# firewall-cmd --zone=public --remove-rich-rule='rule family="ipv4" forward-port to-addr="192.168.7.11" to-port="22" protocol="tcp" port="2022"'
...
# copy cert to helper node
scp /etc/crts/* 192.168.7.11:/root/

# in helper node
cp /root/redhat.ren.crt /etc/pki/ca-trust/source/anchors/
update-ca-trust extract
...
# INFO Access the OpenShift web-console here: https://console-openshift-console.apps.paa.redhat.ren
# INFO Login to the console with user: kubeadmin, password: 8HCX6-2nVpp-PhMhT-twg8B
```
配置一下本地的dns ( 把 *.apps.paa.redhat.ren 配置成 192.168.7.11 ) ，指向工具机的haproxy，打开浏览器就能访问管理界面了
...
oc create configmap ca.for.registry \
    --from-file=registry.ipincloud.com=/root/ocp4/redhat.ren.crt
...
# oc patch image.config.openshift.io/cluster -p '{"spec":{"registrySources":{"insecureRegistries":["registry.ipincloud.com"]}}}'  --type=merge
...
yum -y install python36 jq wget
...
spec:
  ...
  sourceType: grpc
  image: registry.ipincloud.com:5443/docker.io/wangzheng422/custom-registry-redhat:2019-12-30
  publisher: Red Hat
EOF
...
spec:
  ...
  sourceType: grpc
  image: registry.ipincloud.com:5443/docker.io/wangzheng422/custom-registry-certified:2019-12-30
  publisher: Certified
EOF
...
spec:
  ...
  sourceType: grpc
  image: registry.ipincloud.com:5443/docker.io/wangzheng422/custom-registry-community:2019-12-30
  publisher: Community
EOF
